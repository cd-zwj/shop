...
describe('TopNav', () => {
  ...
  it('caps the visible unread notification count at 99+', async () => {
    mockedNotificationService.getUnreadCount.mockResolvedValue({ count: 120 });

    const element = await renderTopNav('user');

    expect(mockedNotificationService.getUnreadCount).toHaveBeenCalledTimes(1);
    expect(element.querySelector('[data-testid="top-nav-notification-count"]')?.textContent).toBe('99+');
  });
